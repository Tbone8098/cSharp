...
class SLList {
    ...
    average() {
        ...
        while (runner != null) {
            // we want to move runner, adding to the sum and length until we finish touching every node
    ...
    removeFromBack() {
        ...
        if (this.isEmpty()) {
            // if the list has no nodes to remove
            ...
            while (runner.next != null) {
                // running until runner.next is null will put runner at the last node, and lagger at the 2nd to last node
    ...
    secondToLast() {
        if (this.isEmpty() || this.head.next == null) {
            // If the list is empty or only 1 element in length, we'll return null
            return null;
        }
        let runner = this.head; // start at the first node
        while (runner.next.next != null) {
            //until runner's .next has a .next of null, we want to move down the line
            runner = runner.next;
        }
    ...
    removeVal(value) {
        if (this.isEmpty()) {
            ...
            while (runner.next != null) {
                // I'm going to use while runner.next != null because I'm going to be checking the next node each step of the way
                if (runner.next.value == value) {
                    // if the next node has our desired value
    ...
    moveMinToFront2() {
        // user runner and lagger to run throught the nodes
        // use lowest var and previousLowest var
        // keep track of current lowest val and node before current lowest val
        // after list is finished set lowest val next to this.head
        // set node before lowest val .next to .next.next 
        // set new head to lowest val
        ...
        let lagger = this.head;

        let lowest = this.head;
        let previousLowest = null;

        while (runner != null) {
            if (runner.value < lowest.value) {
                lowest = runner
                previousLowest = lagger
            }
            runner = runner.next;
            lagger = lagger.next;
        }
        if (this.head.value > lowest.value) {
            previousLowest.next = previousLowest.next.next;
            lowest.next = this.head;
            this.head = lowest;
        }
...
myList.seedFromArr([5, 3, 1]);

let myList2 = new SLList();
myList2.seedFromArr([7, 2, 8])

let myList3 = new SLList();
// myList3.seedFromArr([6, 3, 4, 5, 6, 2, 4, 9]);
// myList3.seedFromArr([1, 6, 3, 4, 5, 6, 2, 4, 9]);
// myList3.seedFromArr([1]);
myList3.seedFromArr([]);

// myList.concat(myList2).printList();
myList3.moveMinToFront2().printList();
